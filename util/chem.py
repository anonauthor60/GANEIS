...
def get_mol_graph(mol, elem_attrs, add_h=False):
    try:
        ...
        bond_lengths = torch.tensor(bond_lengths, dtype=torch.float).view(-1, 1)
        ...
        return Data(x=atom_feats,
                    ...
                    n_atoms=n_atoms,
                    n_atoms_r=n_atoms.repeat(atom_feats.shape[0], 1),
                    bond_lengths=bond_lengths,
                    atom_nums=atom_nums_mol,
                    coords=coords)
    except RuntimeError:
        return None


def draw_mol(mol, path_img):
    Draw.MolToFile(mol, path_img)
